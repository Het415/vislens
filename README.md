...
| CI | **built** — ruff + 271 tests across three jobs, no secrets, no network, no GPU |
...
271 tests (72 fetch controls, 65 rules, 33 service, 30 near-duplicate, 26 catalog build,
14 shard packing, 22 training), no network, no API key and no GPU — which is why the whole suite
runs in CI on every push. The 22 training tests need the `train` extra and skip without it; a
